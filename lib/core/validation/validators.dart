...
class Validators {
	...
	static String? confirmPassword({required String? password, required String? confirm}) {
		final p = password ?? '';
		final c = confirm ?? '';
	...
	static String? listingStartingBid(String? value) => positiveMoney(value, fieldName: 'Starting bid');

	static String? listingBuyoutPrice({required String? buyoutValue, required double? startingBid}) {
		...
		if (start != null && parsed < start) return 'Buyout price must be ≥ starting bid';
		return null;
	}

	static String? bidAmount({required String? value, required double? currentBid, required double? startingBid}) {
		final v = (value ?? '').trim();
